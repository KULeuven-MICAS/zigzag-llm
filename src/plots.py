...
def plot_energy_clean(cmes: list[CME_T], filename: str):
    ...
    sections = (
        PlotCMEDetailed.energy_sections
...
def plot_energy_compare(
    cmes_all: list[list[CME_T]], supergroups: list[str], filename: str, title: str = ""
):
    """`cmes` correspond to `LAYERS_TO_PLOT`"""
    assert all([len(cmes) == 5 for cmes in cmes_all]), (
        "Are these the CMEs from `LAYERS_TO_PLOT`?"
    )
    assert len(cmes_all) == len(supergroups)

    groups = [
        f"{group}\n{supergroup}"
        for supergroup, group in itertools.product(supergroups, GROUPS)
    ]
    ...
    for cmes_per_supergroup in cmes_all:
        data_per_supergroup = PlotCMEDetailed.cmes_to_energy_array_all(
            cmes_per_supergroup
        )
        data = np.concatenate([data, data_per_supergroup], axis=0)

    p = BarPlotter(
        groups, bars, sections, legend_cols=5, ylabel="Energy (pJ)", title=title
    )
    p.plot(data, filename)


def plot_energy_compare_minimal(
    cmes_all: list[list[CME_T]], groups: list[str], filename: str, title: str = ""
):
    ...
    data = np.array(
        [
            PlotCMEMinimal.cmes_to_energy_array_single_group(cmes_single_group)
            for cmes_single_group in cmes_all
        ]
    )
...
def plot_latency_clean(cmes: list[CME_T], filename: str):
    ...
    data = group_results(
        [PlotCMEDetailed.cme_to_latency_array_single_group(cme) for cme in cmes]
    )
    p = BarPlotter(
        GROUPS,
        bars,
...
def plot_latency_compare(
    ...
    for cmes_per_supergroup in cmes_all:
        data_per_supergroup = group_results(
            [
                PlotCMEDetailed.cme_to_latency_array_single_group(cme)
                for cme in cmes_per_supergroup
            ]
        )
        data = np.concatenate([data, data_per_supergroup], axis=0)
...
def plot_latency_compare_minimal(
    ...
    data = np.array(
        [
            PlotCMEMinimal.cmes_to_array_single_group(cmes_single_group)
            for cmes_single_group in cmes_all
        ]
    )
...
def plot_energy_and_latency(
    ...
    for cmes_per_supergroup in cmes_all:
        energy_data_per_supergroup = PlotCMEDetailed.cmes_to_energy_array_all(
            cmes_per_supergroup
        )
    ...
    latency_data = np.array(
        [
            PlotCMEDetailed.cme_to_latency_array_all_single_group(cmes_per_supergroup)
            for cmes_per_supergroup in cmes_all
        ]
    )
...
def plot_energy_and_latency_minimal(
    cmes_all: list[list[CME_T]], groups: list[str], title: str, filename: str
):
    assert all([len(cmes) == 5 for cmes in cmes_all]), (
        "Are these the CMEs from `LAYERS_TO_PLOT`?"
    )
    ...
    latency_data = np.array(
        [
            PlotCMEDetailed.cme_to_latency_array_all_single_group(cmes_single_group)
            for cmes_single_group in cmes_all
        ]
    )
